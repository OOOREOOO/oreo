    ...
    $(document).ready(
        function () {
            ...
            if ($set['theme']['refurbish'] == 1) {
                echo "$(document).pjax('a[target!=_blank]', '#app', {fragment:'#app', timeout:8000});";
                if ($set['theme']['loadbar'] == 1) {
                    echo "$(document).on('pjax:end', function () {NProgress.done();});$(document).on('pjax:start', function () {NProgress.start();});";
                }
            }
            if ($set['theme']['loadbar'] == 1) {
                echo "NProgress.done();";
